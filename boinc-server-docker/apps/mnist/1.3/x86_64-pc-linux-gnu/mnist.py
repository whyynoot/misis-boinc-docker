...
def run(job_file: pathlib.Path, output_file: pathlib.Path):
    ...
    def accuracy(xd, yd, weights, bias):
        probs = softmax(xd @ weights + bias)
        preds = probs.argmax(axis=1)
        return float((preds == yd).mean())

    def xent_loss(xd, yd, weights, bias):
        probs = softmax(xd @ weights + bias)
        logp = -np.log(probs[np.arange(yd.size), yd] + 1e-12)
    ...
    for epoch in range(train_epochs):
        ...
        train_acc_hist.append(accuracy(x_train_s, y_train_s, W, b))
        val_acc_hist.append(accuracy(x_val_s, y_val_s, W, b))
        train_loss_hist.append(xent_loss(x_train_s, y_train_s, W, b))
        val_loss_hist.append(xent_loss(x_val_s, y_val_s, W, b))
    runtime_sec = time.time() - start

    train_acc = train_acc_hist[-1] if train_acc_hist else accuracy(x_train_s, y_train_s, W, b)
    val_acc = val_acc_hist[-1] if val_acc_hist else accuracy(x_val_s, y_val_s, W, b)
